...
def ExportTopOwnPastRatedProdForPortalsUsers(jobid,conn_string):
    ...
    logger = logging.getLogger("ExportTopOwnPastRatedProdForPortalsUsers")

    try:
        df = pd.read_csv('./data/jobid'+str(jobid)+'/predictionfiles/TOP_OWN_PAST_RATED_PROD_FOR_PORTALS_USERS.CSV')
        if df.shape[0] > 0:    
            ...
            try:
                ...
                with engine.connect() as conn:
                    ...
                    try:
                        ...
                        tran = conn.begin()
                        logger.info("Truncating table PAST_RATEITEM_PORTAL_USER!")
                        data = conn.execute("""TRUNCATE TABLE PAST_RATEITEM_PORTAL_USER""")                        
                        logger.info("Inserting data into PAST_RATEITEM_PORTAL_USER!")
                        df.to_sql('PAST_RATEITEM_PORTAL_USER', con = engine, if_exists = 'append', chunksize = 1000,index=False, dtype=dtyp) # takes 45 sec for 100000 records
                        logger.info("Inserting data into PAST_RATEITEM_PORTAL_USER_HIST!")
                        data = conn.execute("""INSERT INTO PAST_RATEITEM_PORTAL_USER_HIST (USER_ID,ITEM_ID,RATING,PORTAL,AIML_JOB_RUN_ID)
